 ...
 * 
 * Versions:
 * 0.5.1 ROB - allow limit to circular region
 *           - remove deprecated jQuery use of .browser
...
(function(Raphael) {
	...
	var SketchPad = function(paper, options) {
		...
		self.setCircularBounds = function(centreX, centreY, boundsRadius) {
			_pen.setCircularBounds(centreX, centreY, boundsRadius);
		}
	...
	var Pen = function() {
		...
		var _hasCircularBounds = false;
		var _centreX = 0;
		var _centreY = 0;
		var _boundsRadius = 10;

		self.setCircularBounds = function(centreX, centreY, boundsRadius) {
			_hasCircularBounds = true;
			_centreX = centreX;
			_centreY = centreY;
		...
		self.pushPoint = function(e, offset) {
			var x = e.pageX - _offset.left,
				y = e.pageY - _offset.top;

			// Check for circular bounds
			if (_hasCircularBounds)
			{
				var x_off = x - _centreX,
					y_off = y - _centreY;
				if ((x_off*x_off + y_off*y_off) > (_boundsRadius*_boundsRadius))
				{
					return false;
				}
			}
